<template>
  <div class="wrap">
    ...
    <div class="postLists">
      <ul>
        <li>
          <span v-for="(sort,index) in sorts" @click="onSort(sort,index)" :class="{selected: index === selected}">{{sort}}</span>
        </li>
        <li v-for="post in posts">
          <div class="start">
            ...
            <span
              :class="[{sort_top:(post.top === true),sort_good:(post.good === true),
                        sort_othersList:(post.top === !true && post.good === !true)},]"
            >{{post | showSort}}</span>
...
export default {
  ...
  methods: {
    ...
    changePage(value) {
      this.needPaga = value.page
      this.tab = value.tab || this.tab
      if (value.selected === 0) {
    ...
    onSort(sort, index){
      ...
      this.fetchData(tab)
      this.selected = index
      this.$root.bus.$emit('changeCurrentButton', 1)
    }
  },
  beforeMount() {
    this.fetchData()
...
ul li:first-child > span.selected {
  padding: 3px 4px;
  border-radius: 3px;
  background: #80bd01;
  color: white;
